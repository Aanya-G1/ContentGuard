...
void DatabaseOperations::Connection(const string& url){
    try{
...
void DatabaseOperations::storeFingerprints(int docId, const vector<uint64_t>& hashes){
    try{
        ...
        for(auto hash: hashes){
            txn.exec("INSERT INTO fingerprints(doc_id,hash_value) VALUES($1,$2)", pqxx::params{docId, hash});
        }
        txn.commit();
...
vector<int> fingerprintProcessing(int docId,vector<std::string> tokens){
    return fingerprintProcessing(docId, tokens,
        "postgresql://neondb_owner:npg_2TJFqNlzPp5Y@"
        "ep-calm-heart-a1fxv4n4-pooler.ap-southeast-1.aws.neon.tech/"
        "contentGuard?sslmode=require&channel_binding=require");
}

vector<int> fingerprintProcessing(int docId, vector<std::string> tokens, const string& connUrl){
    DatabaseOperations db;
    db.Connection(connUrl);
